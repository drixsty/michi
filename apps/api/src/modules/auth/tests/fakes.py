...
class FakeDb:
    """Simule la session SQLAlchemy pour flush()."""
...
class FakeOrganizationRepository:
    ...
    # Méthode supplémentaire appelée par OrgService (leakage temporaire)
    async def save(self, org_model: object) -> object:  # type: ignore[override]
        if hasattr(org_model, "id"):
            if hasattr(org_model, "slug") and isinstance(org_model.slug, str):
                # Modèle ORM direct
                self._store[org_model.id] = org_model  # type: ignore[assignment]
            else:
                self._store[org_model.id] = org_model  # type: ignore[assignment]
        return org_model
