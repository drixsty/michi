...
async def test_complete_auth_and_2fa_flow(db_session):
    ...
    test_user = User(
        ...
        hashed_password=hasher.hash("password123").value,
        is_active=True
    )
    ...
    setup_data = await tf_service.setup_2fa(user_id_str, session=db_session)
    ...
    recovery_codes = await tf_service.confirm_2fa(user_id_str, secret, code, session=db_session)
    assert recovery_codes is not None
    
    # 4. Vérifier que l'utilisateur a bien le 2FA activé en base
    stmt = select(User).where(User.email == "test@michi.com")
