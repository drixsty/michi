"""
Tests unitaires — Run Rate Algorithm (US 2.5)
...
class TestCalculateRunRate:
    ...
    def test_short_series_fallback_global_median(self):
        ...
        # 3 jours seulement (< RUN_RATE_MIN_PERIODS=4)
        df = make_df([8.0, 8.0, 8.0])
        result = calculate_run_rate(df)
        # Doit retourner la médiane globale (8.0) plutôt que NaN
    ...
    def test_stockout_days_excluded(self):
        ...
        # 30j à 10 unités, puis 5j de rupture à 0
        corrected = [10.0] * 30 + [0.0] * 5
        stockout = [False] * 30 + [True] * 5
        df = make_df(corrected, is_stockout=stockout)
        result = calculate_run_rate(df)
        # Le run rate final doit refléter les 10 unités, pas les 0 des ruptures
    ...
    def test_sorted_by_date(self):
        ...
        df = make_df([5.0] * 10)
        df = df.iloc[::-1].reset_index(drop=True)  # inverser
        result = calculate_run_rate(df)
