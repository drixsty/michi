"""
Tests unitaires — Out-of-Stock Correction (US 2.1)
...
class TestCorrectOutOfStock:
    ...
    def test_fallback_to_global_median(self):
        """Si toute la série est en rupture, fallback = 0 (médiane d'une série vide)."""
        units = [0.0] * 10
        stocks = [0] * 10
        df = make_df(units, stocks)
        result = correct_out_of_stock(df)
    ...
    def test_sorted_by_date(self):
        ...
        df = df.iloc[::-1].reset_index(drop=True)  # inverser l'ordre
        result = correct_out_of_stock(df)
