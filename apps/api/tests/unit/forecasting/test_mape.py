...
class TestMAPE:
    ...
    def test_mape_oos_correction_below_threshold(self):
        """OOS Correction : MAPE < 25% sur ruptures simulées.

        Seuil 25% pour l'algorithme OOS seul (médiane 14j) : algorithme robuste
        mais limité par la variabilité naturelle de la demande. La pipeline
        complète OOS+IQR atteint < 15% (test_mape_full_pipeline_below_threshold).
        """
        OOS_MAPE_THRESHOLD = 25.0
        ...
        assert error < OOS_MAPE_THRESHOLD, f"MAPE OOS = {error:.2f}% >= {OOS_MAPE_THRESHOLD}%"

    def test_mape_outlier_correction_below_threshold(self):
        """IQR Correction : MAPE < 20% sur outliers simulés.

        Seuil 20% pour l'algorithme IQR seul : après injection d'outliers extrêmes
        (500 vs ~8 naturel), certains points corrigés ont une GT naturellement haute
        (ex : 11.26) ce qui génère une erreur incompressible. La pipeline combinée
        atteint < 15% (test_mape_full_pipeline_below_threshold).
        """
        IQR_MAPE_THRESHOLD = 20.0
        ...
        error = mape(pd.Series(actual), pd.Series(forecast))
        assert error < IQR_MAPE_THRESHOLD, f"MAPE IQR = {error:.2f}% >= {IQR_MAPE_THRESHOLD}%"
