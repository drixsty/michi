...
from src.modules.inventory.models import Product, PlatformSource, SalesLog
...
async def test_trigger_mock_sync_success():
    ...
    with patch("src.modules.shopify.service.generate_full_mock_dataset", return_value=(mock_products, mock_sales)):
...
async def test_trigger_mock_sync_update_existing():
    ...
    with patch("src.modules.shopify.service.generate_full_mock_dataset", return_value=(mock_products, [])):
        service = ShopifyService(db)
