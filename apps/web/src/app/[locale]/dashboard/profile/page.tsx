...
export default function ProfilePage() {
  ...
  const [googleLogin, { loading: googleLoading }] = useMutation(GOOGLE_LOGIN, {
    ...
    onError: (err) => {
      toast.error(err.message || "Erreur lors de la liaison du compte");
    }
  });
  ...
  const handleGoogleLink = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      googleLogin({ variables: { input: { idToken: tokenResponse.access_token } } });
    },
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 px-4 mt-4 animate-in fade-in duration-500">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 pb-6 border-b border-border gap-4">
        ...
        <main className="space-y-6">
              ...
              <section className="bg-white rounded-lg border border-border overflow-hidden">
                ...
                <div className="p-5 space-y-5">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-5 pb-2">
                    <div className="w-14 h-14 rounded-lg bg-muted flex items-center justify-center text-muted-foreground border border-border text-xl font-bold shrink-0">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full h-11 px-3 rounded-lg border border-border bg-background focus:ring-2 focus:ring-primary/10 focus:border-primary/20 transition-all outline-none text-sm"
                      />
                    </div>
                    <div className="space-y-1.5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        value={email}
                        disabled
                        className="w-full h-11 pl-9 pr-4 rounded-lg border border-border bg-muted/50 text-muted-foreground cursor-not-allowed text-sm"
                      />
                    </div>
                  </div>
                  ...
                  <div className="pt-2 flex flex-col sm:flex-row justify-end gap-3">
                    <button
                      ...
                      disabled={updating}
                      className="h-11 px-6 bg-foreground text-background rounded-lg text-[13px] font-semibold hover:opacity-90 transition-all flex items-center justify-center gap-2 w-full sm:w-auto whitespace-nowrap"
                    >
                      {updating ? (
          ...
          {activeTab === 'security' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              {/* Linked Accounts - Premium Flat Design (Sentence Case & Radius LG) */}
              <section className="bg-white rounded-lg border border-border shadow-sm overflow-hidden">
                <div className="px-5 py-3.5 border-b border-border bg-slate-50/50 flex items-center justify-between">
                  <h2 className="text-[12px] font-bold text-foreground/70 tracking-tight">Comptes liés</h2>
                  <Shield className="h-3.5 w-3.5 text-muted-foreground/30" />
                </div>
                <div className="p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-lg border border-slate-100 bg-slate-50/30 hover:border-primary/20 transition-all group gap-4">
                    <div className="flex items-center gap-4">
                      <div className="w-11 h-11 rounded-lg bg-white flex items-center justify-center border border-slate-200 shadow-sm group-hover:scale-105 transition-transform shrink-0">
                      ...
                      <div className="space-y-0.5">
                        <p className="text-sm font-bold text-foreground">Compte Google</p>
                        <p className="text-[11px] text-muted-foreground font-medium">
                          {data?.me?.googleId ? 'Votre identité est vérifiée et liée' : 'Connectez votre compte pour un accès rapide'}
                        </p>
                      </div>
                    </div>

                    {data?.me?.googleId ? (
                      <div className="flex items-center justify-center sm:justify-start gap-2 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-100 shadow-sm shrink-0 w-full sm:w-auto">
                        <CheckCircle2 className="h-3.5 w-3.5" />
                        <span className="text-[10px] font-bold tracking-tight">Compte lié</span>
                      </div>
                    ) : (
                      <button
                        onClick={() => handleGoogleLink()}
                        disabled={googleLoading}
                        className="w-full h-11 flex items-center justify-center gap-3 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-all group"
                      >
                        ...
                        <span className="text-sm font-semibold text-slate-700">Continuer avec Google</span>
                        {googleLoading && <div className="h-4 w-4 border-2 border-slate-300 border-t-slate-600 rounded-full animate-spin ml-2" />}
                      </button>
                    )}
                  </div>
                </div>
              ...
              <section className="bg-white rounded-lg border border-border overflow-hidden">
                ...
                <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <button
                    ...
                    disabled={exporting}
                    className="h-9 px-4 border border-border rounded-lg text-[12px] font-semibold hover:bg-muted transition-all shrink-0 flex items-center justify-center gap-2 w-full sm:w-auto"
                  >
                    {exporting ? (
              ...
              {/* Danger Zone */}
              <div className="pt-2">
                <div className="bg-red-50/50 rounded-lg border border-red-100 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <button
                    ...
                    disabled={deleting}
                    className="h-9 px-4 bg-red-600 text-white rounded-lg text-[12px] font-bold hover:bg-red-700 transition-all shrink-0 flex items-center justify-center gap-2 shadow-sm w-full sm:w-auto"
                  >
                    {t('deleteAccount')}
                  </button>
                </div>
                ...
                <div className="p-5 space-y-8">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <button
                      ...
                      className={cn(
                        "w-9 h-5 rounded-full transition-all relative flex items-center px-0.5 border border-border shrink-0",
                  ...
                  <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      ...
                      <span className={cn(
                        "text-[10px] font-bold px-2 py-0.5 rounded border uppercase w-fit",
